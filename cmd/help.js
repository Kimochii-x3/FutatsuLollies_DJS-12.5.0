...
module.exports = {
    ...
    async execute (bot, message, args, option, commands, prefix) {
        ...
        if (args[0] === 'cmds') {
            ...
            return message.channel.send(cmdsEmbed).catch(bot.errHandle);
        } else if (args[0] === 'desc') {
            const descEmbed = new Discord.MessageEmbed()
            .setColor('GREY')
            .setAuthor('Descriptions:')
            .setDescription(dataDesc);
            return message.channel.send(descEmbed).catch(bot.errHandle);
        } else if (args[0] === 'info') {
            ...
            return message.channel.send(infoEmbed).catch(bot.errHandle);
        } else if(!args[0]) {
            ...
            return message.channel.send(helpEmbed).catch(bot.errHandle);
        }
    },
};