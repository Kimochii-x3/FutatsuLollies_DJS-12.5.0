...
module.exports = {
    ...
    async execute (bot, message, args, option, commands, prefix, errorLogs) {
        ...
        if (!botPerms) {
            return message.channel.send('I do not have one or more of these permissions: `Send Messages; Manage Roles; Embed Links; Add Reactions;` to my role `FutatsuLollies`').catch(err => errorLogs.send(err));
        } else if (botPerms) {
            ...
            if (!phC) {
                try {
                    message.guild.roles.create({ data: { name: '▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇' }, reason: 'Creating a placeholder role to preview the colors'});
                ...
                if (!idOthers) {
                    if (!roleColor) {
                        if (!hexCode) {
                            ...
                            return message.channel.send('Incorrect hexcode, example: `prefix`.color #ff00ff')
                        } else if (hexCode.startsWith('#')) {
                                    ...
                                    await botMsg.awaitReactions(filter, { max: 1, time: 12000, errors: ['time'] }).then(async reacts => {
                                        ...
                                        if (reaction.emoji.name === '✅') {
                                            ...
                                            message.guild.roles.create({ data: { name: `USER-${message.author.id}`, color: hexCode, position: placeholder.position +1 } }).then(async userRole => {
                                                ...
                                                botMsg.edit(acceptEmbed).then(async botMsgDelete => { botMsgDelete.delete( { timeout: 2000 } ).catch(err => errorLogs.send(err)); }).catch(err => errorLogs.send(err));
                                            });
                                            ...
                                            .setColor(message.member.displayHexColor);
                                            botMsg.edit(cancelEmbed).then(async botMsgDelete => { botMsgDelete.delete( { timeout: 2000 } ).catch(err => errorLogs.send(err)); }).catch(err => errorLogs.send(err));
                                        }
                                    }).catch(() => {
                                        const latestEmbed = botMsg.embeds[0];
                                        ...
                                        botMsg.edit(noResponseEmbed).then(async botMsgDelete => { botMsgDelete.delete( { timeout: 2000 } ).catch(err => errorLogs.send(err)); }).catch(err => errorLogs.send(err));
                                    });
                                }).catch(err => errorLogs.send(err));
                            }).catch(err => errorLogs.send(err));
                        }
                        ...
                        if (!hexCode) {
                            ...
                            roleColor.delete('User requested to delete their custom role').catch(err => errorLogs.send(err));
                                ...
                                message.channel.send(rolePreview).then(async botMsg => {
                                    ...
                                    await botMsg.awaitReactions(filter, { max: 1, time: 12000, errors: ['time'] }).then(async reacts => {
                                        ...
                                        if (reaction.emoji.name === '✅') {
                                            ...
                                            .setColor(message.member.displayHexColor);
                                            roleColor.setColor(hexCode).catch(err => errorLogs.send(err));
                                            botMsg.edit(acceptEmbed).then(async botMsgDelete => { botMsgDelete.delete( { timeout: 2000 } ).catch(err => errorLogs.send(err)); }).catch(err => errorLogs.send(err));
                                        } else if (reaction.emoji.name === '❌') {
                                            const latestEmbed = botMsg.embeds[0];
                                            const cancelEmbed = new Discord.MessageEmbed(latestEmbed)
                    ...
                    if (hexCode === `<@!${idOthers.id}`) {
                        ...
                        if (!roleColorOthers) {
                            return message.channel.send(`No user role found, however the highest role's hexcode is: ${idOthers.displayHexColor}`);
                        } else if (roleColorOthers) {
                            return message.channel.send(`User role hexcode is: ${roleColorOthers.hexColor}`)
                        }
                    }
                }
            }
